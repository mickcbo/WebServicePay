	...
	public static void getAuthentification(String nom, String prenom,
		...
		if (verifCarte.getErrors().isEmpty()) {
			...
			try {
				...
				Logger.info("message facturaction : %s",
		...
		Transaction trans = Transaction.find("id_commande=?", id_commande)
				.first();
		if (trans == null) {
			Integer.valueOf(id_commande);
			trans = new Transaction(id_commande, verifCarte.getErrors()
					.isEmpty(), new Date(), nom, prenom);
		} else {
		...
		Commande commande = new Commande(verifCarte.getErrors().isEmpty(),
				verifCarte.getErrors());
		renderJSON(commande);
